...
class Queen(Piece):
    ...
    def _is_valid_move(self, board, row, col):
        ...
        if not(row < 0 or row > 7) and not(col < 0 or col > 7):
            #check the queen is moving into an empty space or one occupied by adversary piece
            if board[row][col] == None or board[row][col].color =='w':
                #check if queen is moving like a rook in a staight line t shape (forward, backward, sideways)
                dx = abs(self.row-row)
                dy = abs(self.col-col)
                if dx != 0 and dy == 0:
                    #check if another piece blocks the trayectory (queen cant jump)
                    blocked = 0
                    ...
                    if blocked == 0:
                        ...
                        print("Invalid movement")
                        return False
                #check if queen is moving like a bishop in a diagonal line x shape
                elif dx == dy:
                    blocked = 0
                    #check if another piece blocks the trayectory (queen cant jump)
                    for i in range (dx):
